...
class MockServo:
    ...
    def read_all(self, sids):
        """(position, speed, load) per servo, mirroring the real driver's single
        sync-read. Same no-None rule as read_positions."""
        return [(self.read_position(s), self.read_speed(s), self.read_load(s))
                for s in sids]
