...
{
	rFlushSCS();
	writeBuf(ID, 0, NULL, 0, INST_ACTION);
	wFlushSCS();
	return Ack(ID);
}
...
int SCS::Recal(u8 ID)
{
	// The Feetech SCS/SMS protocol has no CAL opcode; the only reset-style
	// instruction is INST_RESET (0x06). INST_CAL was an invention in this
	// header; routing to INST_RESET keeps the function usable without a
	// made-up wire value.
	rFlushSCS();
	writeBuf(ID, 0, NULL, 0, INST_RESET);
	wFlushSCS();
