...
# The model ships in this repo (`assets/cartesian_hand/`) -- generated by
# `cartesian_hand_creation.py` in the `legged_env_v2` repo from CAD kinematics,
# and synced in here with `scripts/sync_sim_asset.py` rather than hand-edited.
# `LEGGED_ENV_ROOT` overrides this with a sibling `legged_env_v2` checkout's
# copy instead, for iterating on the model itself.
LEGGED_ENV = os.environ.get("LEGGED_ENV_ROOT")

DEFAULT_MJCF = (
    os.path.join(LEGGED_ENV, "asset", "cartesian_hand", "cartesian_hand.xml")
    if LEGGED_ENV else
