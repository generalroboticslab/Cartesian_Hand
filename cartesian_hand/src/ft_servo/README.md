...
servos** — torque comes on at start and off at exit.
...
65°C — a servo that browns out mid-write answers to no ID at all afterwards.
Writes EPROM, survives power cycles; afterwards the only way to find the servo
is to scan for it.
...
On success it releases torque, rescans, and rebuilds the panel — a written ID
...
`span` and `torque` are guards, not preferences — this tool cannot know what a
...
current target — a servo stalled by too low a cap moves the moment you raise it.
...
Every method carries a docstring, so `help(FtServo)` is the reference. This is
the map.
...
Writes are the whole story. `set_positions` is a broadcast nobody acknowledges,
...
throws while the poll thread runs — they share one buffer, so use one or the
other.
...
is the discriminator: consecutive sweeps in which *no* servo replied. One or two
is interference; climbing is a dead bus. The GUI releases and exits after two
seconds of it.
